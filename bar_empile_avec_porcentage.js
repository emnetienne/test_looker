...
/**
 * Répartition du CA avec % — Looker Studio Community Visualization
 * -----------------------------------------------------------------
 * Ce fichier contient UNIQUEMENT le code de la visualisation.
 * Il doit être concaténé avec dscc.min.js avant l'upload (voir README).
 *
 * Champs attendus (voir viz-config.json) :
 *  - dimension : la catégorie / le segment (ex: "CA Exposé", "CA Incrémental")
 *  - metric    : la valeur (CA en €) associée à chaque catégorie
 */

(function () {
  var CONTAINER_ID = 'viz-container';
  ...
  function drawViz(data) {
    // 1. On vide le conteneur à chaque redraw (obligatoire)
    container.innerHTML = '';
    ...
    // 2. Lecture des options de style (avec valeurs par défaut)
    var colorPrimary = (style.colorPrimary && style.colorPrimary.value && style.colorPrimary.value.color) || '#2E4FA3';
    var colorSecondary = (style.colorSecondary && style.colorSecondary.value && style.colorSecondary.value.color) || '#FF6F59';
    var showTitle = style.showTitle ? style.showTitle.value : true;
    var chartTitle = (style.chartTitle && style.chartTitle.value) || 'Répartition du chiffre d\'affaires généré';
    var palette = [colorPrimary, colorSecondary, '#34A853', '#FBBC04', '#9C27B0', '#00ACC1'];

    // 3. Extraction dimension + métrique par ligne
    ...
    if (showTitle) {
      ...
      titleEl.style.cssText = 'font-size:14px;color:#5f6368;text-align:center;margin-bottom:12px;';
      wrapper.appendChild(titleEl);
    }

    var barContainer = document.createElement('div');
    barContainer.style.cssText = 'flex:1;display:flex;flex-direction:column;border-radius:6px;overflow:hidden;';

    segments.forEach(function (seg, i) {
      var pct = total > 0 ? seg.value / total : 0;
      var segEl = document.createElement('div');
      // flex-grow proportionnel au %, avec un minimum pour rester lisible
      ...
      segEl.textContent = formatEuro(seg.value) + '  (' + formatPct(pct) + ')';
  ...
  // 5. Abonnement aux données Looker Studio
