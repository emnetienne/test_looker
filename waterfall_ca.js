/**
 * ============================================================================
 *  Répartition du CA par segment (Waterfall) — Visualisation custom Looker
 * ----------------------------------------------------------------------------
 *  Waterfall cumulatif qui reprend le design du PPT (slides 2 & 3) :
 *    - Barres incrémentales orange     #FF755D
 *    - Barre "Total" bleu périwinkle   #B6C1E9
 *    - Libellés bleu marine            #1F2D4D
 *
 *  Chaque barre affiche sa VALEUR (arrondie, sans décimale) et son
 *  POURCENTAGE du total.
 *  Titre masqué par défaut (déjà présent sur la slide) — réactivable via l'option.
 *
 *  Données attendues : 1 dimension (segment) + 1 mesure (CA).
 * ============================================================================
 */

looker.plugins.visualizations.add({
  id: "waterfall_ca",
  label: "Répartition du CA par segment (Waterfall)",
  ...
  updateAsync: function (data, element, config, queryResponse, details, done) {
    ...
    if (!dims.length || !meas.length) {
      this.addError({
        title: "Champs requis",
        message: "Ajoutez 1 dimension (segment) et 1 mesure (CA)."
      });
      done();
      return;
    }
  ...
  _render: function () {
    ...
    // --- Options ---------------------------------------------------------
    var barColor   = config.bar_color   || "#FF755D";
    var totalColor = config.total_color || "#B6C1E9";
    ...
    var segLblSize = (config.segment_label_size != null && Number(config.segment_label_size) > 0)
      ? Number(config.segment_label_size) : 12.5;
    ...
    // --- Marges & échelle ------------------------------------------------
    // La zone réservée aux libellés de segment s'adapte à la taille de police choisie.
    var mTop = 30, mBottom = showLbl ? Math.max(40, segLblSize * 3.4 + 12) : 16, mL = 12, mR = 12;
    ...
    for (var j = 0; j < n; j++) {
      ...
      // libellé de segment (axe X) — taille pilotée par l'option "segment_label_size"
      if (showLbl) {
        var maxChars = Math.max(6, Math.floor(slot / (segLblSize * 0.62)));
        var lines = wrap(s.label, maxChars);
        var lineHeight = segLblSize * 1.2;
        var ly = plotBottom + segLblSize + 5;
        for (var k = 0; k < lines.length; k++) {
          svg += '<text x="' + cxj + '" y="' + (ly + k * lineHeight) + '" text-anchor="middle" ' +
                 'font-family="Inter" font-weight="' + (s.total ? 700 : 600) + '" ' +
                 'font-size="' + segLblSize + '" fill="' + INK + '">' + esc(lines[k]) + '</text>';
        }
      }
    }
